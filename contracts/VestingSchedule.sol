...
contract VestingSchedule is Ownable {
  ...
  function getTimes() public view returns (uint8) {
    return _times;
  }
  ...
  function addInvestor(address investorAddress, uint256 investmentAmount)
    public
    onlyOwner
  {
    require(
      block.timestamp < _startTime || _startTime == 0,
      "Overtime for registration"
    );
    require(
      _getAllowance() >= _getTotalCoin() + investmentAmount,
      "Exceed allowance"
    );
  ...
  function getInvestorByAddress(address investorAddress)
    public
    view
  ...
  function sendCoinForInvestor() public onlyOwner {
    ...
    require(
      block.timestamp > _startTime + (_count + 1) * _lockTime * 1 minutes,
      "Not until unlock time"
    );
    _count++;
    for (uint256 i = 0; i < _investors.length; i++) {
